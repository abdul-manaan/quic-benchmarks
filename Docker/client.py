...
def run_docker(client: str, url: str, dirpath: str, i: int) -> float:
    ...
    if client == 'curl_h2':
        ...
        print("r->",tshark_output.stderr)
        with open(Path.joinpath(dirpath, f'{client}_{i}.json'), mode='w') as f:
            json.dump(json.loads(tshark_output.stdout.decode()), f)

        result = process_pcap(Path.joinpath(dirpath, f'{client}_{i}.json'))
        os.remove(Path.joinpath(TMP_PCAP, 'out.pcapng'))
        # out_arr = output.stdout.decode().split('\n')[:-1]
        # dns_time = float(out_arr[0].split(':')[1])
        # total_time = float(out_arr[1].split(':')[1])
        # result['time'] = total_time - dns_time
    ...
    logpath = Path.joinpath(
        Path(TMP_QLOG), os.listdir(TMP_QLOG)[0])
    ...
    if dirpath is None:
        print(os.remove(logpath))
    elif client.count('chrome') > 0:
        filepath = Path.joinpath(dirpath, '{}_{}.json'.format(client, i))
        # print(os.remove(logpath))
        print(os.rename(logpath, filepath))
        print("=>",logpath, dirpath, filepath)
    else:
...
def process_sqlog(qlog: str) -> dict:
    with open(qlog, mode='r') as f:
        data = f.read().split("\n")
        data = list(map(lambda x: json.loads("".join(filter(lambda x: x in string.printable, x))) if len(x) > 0 and "{" in x else {}, data))
        events = data[1:]
        time_units = 'ms'
        ...
        for event in events:
            ...
            if event_type.lower() == 'transport:packet_sent' and start is None:
                start = ts

            if start is None:
                continue

            if event_type.lower() == 'transport:packet_received':
                if init_rtt is None:
                    init_rtt = ts - start
